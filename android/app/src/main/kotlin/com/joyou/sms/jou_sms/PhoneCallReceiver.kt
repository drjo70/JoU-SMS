...
class PhoneCallReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (state) {
            ...
            TelephonyManager.EXTRA_STATE_IDLE -> {
                ...
                isIncoming = false
                savedNumber = null
                Log.d(TAG, "🔄 [v0.2.3] 상태 초기화 완료 (다음 전화 대기)")
            }
        }
    ...
    private fun getLastIncomingNumber(context: Context): String? {
        Log.d(TAG, "🔍 [v0.2.3] CallLog에서 최근 통화 번호 조회...")
            ...
            cursor?.use {
                if (it.moveToFirst()) {
                    ...
                    if (numberIndex >= 0) {
                        ...
                        Log.d(TAG, "✅ [v0.2.3] CallLog 번호 찾음: $number")
                        return number
                    }
                }
            }
        } catch (e: Exception) {
            Log.d(TAG, "❌ [v0.2.3] CallLog 조회 실패: ${e.message}")
        }
    ...
    private fun sendSms(context: Context, phoneNumber: String) {
        try {
            Log.d(TAG, "🔧 [v0.2.3] sendSms() 시작!")
            Log.d(TAG, "  - 받는 사람: $phoneNumber")
            
            val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            
            Log.d(TAG, "📂 [v0.2.3] SharedPreferences 파일 확인:")
            ...
            Log.d(TAG, "🔑 [v0.2.3] 저장된 모든 키: $allKeys")
            
            // 설정 확인
            val enabled = prefs.getBoolean(KEY_ENABLED, false)
            val message = prefs.getString(KEY_MESSAGE, null)
            
            Log.d(TAG, "⚙️ [v0.2.3] 설정 값 확인:")
            ...
            if (!enabled) {
                Log.d(TAG, "⏸️⏸️⏸️ [v0.2.3] 자동발송이 꺼져있습니다!")
            ...
            Log.d(TAG, "✅✅✅ [v0.2.3] SMS 발송 완료!")
            
        } catch (e: Exception) {
            Log.d(TAG, "❌❌❌ [v0.2.3] SMS 발송 실패!")
